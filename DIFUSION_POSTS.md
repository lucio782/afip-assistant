# Borradores de difusión — ARCA Assistant

Copiá y pegá. NO spamees: 1 post por comunidad, respondé todos los comentarios, esperá días entre uno y otro.

## 1) Reddit — r/merval (mejor opción para arrancar)

**Título:** Hice una calculadora de Monotributo 2026 gratis (y va con dólar blue y control de gastos)

**Cuerpo:**
Hola gente, soy Lucio. Les comparto algo que estuve armando para monotributistas y gente que arranca con ARCA (ex AFIP): **ARCA Assistant** (https://calculararca.duckdns.org).

Qué hace:
- Te dice tu categoría de Monotributo según ingresos, superficie, energía y alquiler (tabla 2026 actualizada)
- Simulador de recategorización y calendario de vencimientos
- Dólar blue, oficial, tarjeta, MEP y CCL en vivo con histórico
- Registro de gastos ilimitado (ARS/USD) con resumen mensual y exportación a CSV
- Alertas de vencimiento y períodos de recategorización
- Calculadoras de sueldo neto, inflación y préstamos

Es **100% gratis y sin límites** (no hay versión paga ni planes). Los datos se guardan por cuenta y son privados.

No es un sitio oficial de ARCA (ex AFIP): los valores son orientativos, pero uso las tablas vigentes.

Si les sirve, genial; si encuentran algo que no funciona o falta, me dicen y lo arreglo. Cualquier crítica constructiva es bienvenida 🙏

## 2) Reddit — r/argentina

**Título:** Para los que facturan como monotributistas: calculadora de categoría + vencimientos, gratis

**Cuerpo:** (mismo que arriba, más corto, enfatizando lo de "recién arranco y no entiendo nada")

## 3) Facebook — grupos de "Monotributo Argentina", "Emprendedores", "Cuentapropistas"

**Texto:**
¿No sabés en qué categoría de Monotributo estás? ¿Cuándo vence el pago? Te comparto una herramienta gratuita que hice (soy Lucio): entrás tus ingresos y te dice tu categoría 2026, cuánto pagás y cuándo vence. También tiene dólar en vivo y registro de gastos. 100% gratis y sin límites → https://calculararca.duckdns.org

## 4) X/Twitter — hilo corto

**Post 1:** Calculadora de Monotributo 2026 gratis (la hice yo, Lucio): sabé tu categoría, cuánto pagás y cuándo vence en segundos. 🇦🇷 → https://calculararca.duckdns.org
**Post 2:** Además: dólar blue en vivo, gastos ilimitados, alertas de vencimiento, sueldo neto e inflación. Todo gratis, sin planes ni límites.

## 5) LinkedIn — artículo corto

**Título:** Monotributo 2026: las 3 preguntas que todo monotributista se hace (y dónde responderlas)

**Cuerpo:** Artículo breve con la recategorización de julio/enero, qué es la categoría y cómo se calcula; cerrá con el link a la herramienta como recurso gratuito.

## Reglas (del plan)
- 1 aporte por comunidad, responder TODOS los comentarios (eso es lo que da visibilidad)
- Esperá 2-3 días entre publicaciones en distintos lugares
- Si te piden una función, anotala en el repo (issues) y avisá cuando esté
- No pagues publicidad todavía
